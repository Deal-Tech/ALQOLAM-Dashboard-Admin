...
use App\Filament\Dosen\Resources\RespondAssetDesaResource\Pages;
use App\Filament\Dosen\Resources\RespondAssetDesaResource\RelationManagers;
...
class RespondAssetDesaResource extends Resource
{
        ...
        return $form
            ->schema([
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Section::make('Informasi Aset Desa')
                            ->schema([
    ...
    public static function table(Table $table): Table
    {
        ...
        ->query(function () {
            ...
            // Find respond_assetdesa entries where user_id matches entries in respondsurvey with matching dosenpendamping_id
            return RespondAssetDesa::query()
                ->whereHas('user', function (Builder $query) use ($dosenId) {
                    $query->whereHas('respondSurveys', function (Builder $subQuery) use ($dosenId) {
                        $subQuery->where('dosenpendamping_id', $dosenId);
                    });
                });
        })
            ->columns([
